...
def parse_args():
    ...
    parser.add_argument(
        "--instance_has_mask"
        action="store_true",
        help="Flag to indicate if the instance images have masks. If not, random masks will be generated.",
    )
    parser.add_argument(
        "--class_data_dir",
        type=str,
...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        example["mask"] = None
        example["has_mask"] = False
        if self.instance_has_mask:
            # if the instance image has a mask at channel 4, we use it to mask the background
            instance_image = instance_image[:, :, :3]
            example["mask"] = instance_image[:, :, 3]
            example["has_mask"] = True
...
########################## -------New approach---------- ##########################
    # add LoRA layers using peft instead of previous approach
    if args.use_lora:
        config = LoraConfig(
            r=args.lora_r,
    ...
    train_dataset = DreamBoothDataset(
        ...
        instance_has_mask=args.instance_has_mask,
    )
    ...
    def collate_fn(examples):
        ...
        for example in examples:
            ...
            if example["has_mask"]:
                # if the instance has a customized mask, use it
                mask = example["mask"]
            else:
